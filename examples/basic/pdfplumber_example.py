...
def main():
    ...
    output_md_path = Path("outputs/pdfplumber_example_output.md")
    
    # Make sure output directory exists
    output_md_path.parent.mkdir(parents=True, exist_ok=True)
    
    print(f"Parsing document: {pdf_path}")
    print("This will use the pdfplumber parser...")

    # 2. Configure the parser
    # We specify PipelineType.pdfplumber to use the PdfPlumberParser
    config = ParserConfig(pipeline=PipelineType.pdfplumber)
    ...
    for key, value in parsed_doc.metadata.items():
        if key != "Creator": # Some metadata can be verbose
            print(f"  {key}: {value}")
